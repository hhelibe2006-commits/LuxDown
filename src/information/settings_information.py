...
class SettingsManager:
    ...
    def _update_from_dict(self, settings: dict) -> None:
        ...
        if (
            isinstance(settings.get("audio_format"), str)
            and settings.get("audio_format") in self.audio_formats
        ):
            self.current_audio_format = cast(str, settings.get("audio_format"))

        if (
            isinstance(settings.get("video_format"), str)
            and settings.get("video_format") in self.video_formats
        ):
